<h1 align="center">Hi 👋, I'm Tanishka Dabas</h1>

<h3 align="center">Data Analyst • Backend Developer • AI Enthusiast</h3>

<p align="center">
<img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=24&duration=3000&pause=1000&color=6C63FF&center=true&vCenter=true&width=900&lines=B.Tech+Data+Science+Graduate;Data+Analyst;FastAPI+Backend+Developer;Power+BI+Developer;Python+%7C+SQL+%7C+AWS;Always+Learning+Something+New+🚀"/>
</p>

---

## 👩‍💻 About Me

🎓 **B.Tech Graduate in Data Science**

📊 Passionate about **Data Analytics, Backend Development, and Artificial Intelligence**, with hands-on experience in building data-driven solutions and scalable backend applications.

💻 Currently working on **HealthBridge**, a healthcare platform built using **FastAPI** and **React**.

🌱 Currently Learning

- Advanced FastAPI
- Machine Learning
- AWS Cloud
- Backend System Design

💬 Ask me about

- Python
- SQL
- Power BI
- FastAPI
- REST APIs
- Pandas
- Git & GitHub

⚡ **Fun Fact**

I enjoy transforming raw data into meaningful insights and building backend systems that solve real-world problems.

---

# 🌐 Connect With Me

<p align="center">

<a href="YOUR_LINKEDIN_URL">
<img src="https://skillicons.dev/icons?i=linkedin" height="45"/>
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://skillicons.dev/icons?i=gmail" height="45"/>
</a>

<a href="YOUR_PORTFOLIO_URL">
<img src="https://skillicons.dev/icons?i=github" height="45"/>
</a>

</p>

---

# 🚀 Tech Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=python,fastapi,react,vite,html,css,js,mysql,sqlite,aws,git,github,postman,vscode"/>

</p>

---

# 📊 Data Analytics Stack

<p align="center">

Python • SQL • Power BI • Excel • Tableau • Pandas • NumPy • Scikit-Learn • Matplotlib • Databricks

</p>

---

# 🏆 Certifications

| Certification | Issuer |
|---------------|--------|
| Deloitte Data Analytics Job Simulation | Forage |
| Infosys Springboard Internship | Infosys |
| IBM Certification | IBM |
| GeeksforGeeks SkillUp Program | GeeksforGeeks |
| Myntra Data Challenge | Naukri Campus |

---

# 🚀 Featured Projects

## 🏥 HealthBridge

Healthcare Appointment Booking Platform

**Tech Stack**

FastAPI • React • JWT • REST APIs

---

## ⚡ PowerPulse

Electricity Demand Forecasting using Time Series Analysis

**Tech Stack**

Python • Pandas • Machine Learning

---

## 📈 Customer Churn Prediction

Machine Learning model to predict customer churn.

**Tech Stack**

Python • Pandas • Scikit-Learn

---

## 🌦 Weather Forecast API

Weather dashboard using OpenWeatherMap API.

**Tech Stack**

Python • Streamlit • REST APIs

---

## 🎬 Netflix Dashboard

Interactive dashboard developed in Power BI.

---

## 🍽 Zomato Analytics Dashboard

Restaurant Insights using SQL & Power BI.

---

# 📈 GitHub Statistics

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight"/>

</p>

---

# 🔥 GitHub Streak

<p align="center">

<img src="https://streak-stats.demolab.com?user=YOUR_USERNAME&theme=tokyonight"/>

</p>

---

# 📊 GitHub Activity Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=YOUR_USERNAME&theme=tokyo-night"/>

</p>

---

# 👀 Profile Views

<p align="center">

<img src="https://komarev.com/ghpvc/?username=YOUR_USERNAME&label=Profile+Views&color=6C63FF&style=for-the-badge"/>

</p>

---

# 🌱 Currently Exploring

- Backend Architecture
- API Development
- Machine Learning
- Cloud Computing (AWS)
- Data Engineering
- Docker

---

# 💬 Favourite Quote

> **"Without data, you're just another person with an opinion."**  
> — W. Edwards Deming

---

<h3 align="center">

✨ Thanks for visiting my profile! ✨

If you like my work, don't forget to ⭐ my repositories.

</h3>
